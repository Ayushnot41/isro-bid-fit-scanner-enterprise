...
import { Rocket, ShieldCheck, ArrowRight, Lock, Mail, Building2, Sparkles, Award } from "lucide-react";
import { Button } from "@/components/ui/button";

export default function RegisterPage() {
  const [enterpriseName, setEnterpriseName] = useState("AeroPrecision Engineering Pvt Ltd");
  const [udyamNumber, setUdyamNumber] = useState("UDYAM-KR-03-0098421");
  const [email, setEmail] = useState("admin@aeroprecision.in");
        ...
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-[11px] font-mono">
              Aerospace Enterprise MSME Onboarding
            </p>
          </div>
        </Link>
        ...
        <div className="w-full bg-[#0e1115] border border-[#222730] rounded-2xl shadow-2xl shadow-black/80 p-6 sm:p-8 space-y-6">
          <div className="space-y-1 text-center">
            <h2 className="text-xl font-bold text-white tracking-tight">
              Register MSME Enterprise
            </h2>
            <p className="text-xs text-zinc-400 font-mono">
              Onboard your workshop for GFR 170(i) EMD exemptions
            </p>
          </div>
          ...
          >
            ...
            <span>1-Click Verified MSME Onboarding</span>
          </Button>

          <div className="flex items-center gap-3">
            <div className="flex-1 h-px bg-[#222730]" />
            <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider">
              Or Register Workshop
            </span>
            <div className="flex-1 h-px bg-[#222730]" />
          </div>

          {/* Form */}
          <form onSubmit={handleRegister} className="space-y-3.5 text-xs font-mono">
            <div className="space-y-1.5">
              <label className="text-zinc-300 flex items-center gap-1.5">
                <Building2 className="w-3.5 h-3.5 text-emerald-400" />
                <span>Aerospace Enterprise Name</span>
              </label>
              <input
                type="text"
                required
                ...
                placeholder="e.g. AeroTech Systems Pvt Ltd"
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-[#090b0e] border border-[#222730] text-white placeholder-zinc-500 focus:outline-none focus:border-emerald-500/50 font-sans text-xs transition-colors uppercase"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="director@aerotech.in"
                className="w-full px-3.5 py-2.5 rounded-xl bg-[#090b0e] border border-[#222730] text-white placeholder-zinc-500 focus:outline-none focus:border-emerald-500/50 font-sans text-xs transition-colors"
              />
            </div>
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full py-2.5 rounded-xl bg-[#14181f] hover:bg-[#1d232c] border border-[#222730] text-zinc-200 hover:text-white font-mono text-xs font-semibold flex items-center justify-center gap-2 transition-colors cursor-pointer mt-2"
            >
              <ShieldCheck className="w-4 h-4 text-emerald-400" />
              <span>Create Account & Launch HUD</span>
